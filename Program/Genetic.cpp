...
{
    Individual modifiedParent1 = parent1; // Copie du parent 1
    Individual modifiedParent2 = parent2; // Copie du parent 2

    int currentParentIndex = 1; // Commencer avec le parent 1
    int resultRouteIndex = 0; // Index pour suivre la position de la route dans result.chromR

    // Nettoyer les routes de result.chromR
    for (int i = 0; i < params.nbVehicles; i++) {
        result.chromR[i].clear();
    }

    // Sélectionner les routes avec le meilleur ratio bénéfice/coût
    while (true) {

        // Vérifier si le nombre de routes remplies dépasse le nombre de véhicules disponibles
        if (resultRouteIndex >= params.nbVehicles) {
            break; // Sortir de la boucle si le nombre de routes remplies dépasse le nombre de véhicules disponibles
        }
        // Sélection du parent courant
        const Individual &currentParent = (currentParentIndex == 1) ? modifiedParent1 : modifiedParent2;

        // Recherche de la route avec le ratio bénéfice/coût le plus élevé dans le parent courant
        double maxRatio = 0.0;
        int maxRouteIndex = -1;
        for (int i = 0; i < params.nbVehicles; ++i) {
            double routeCost = 0.0; // Initialisation du coût de la route
            double routeBenefit = 0.0; // Initialisation du bénéfice de la route

            // Vérifier si la route a au moins deux clients
            if (currentParent.chromR[i].size() < 2) {
                continue; // Passer à la prochaine itération si la route est vide ou contient un seul client
            }

            // Calcul du coût de la route
            for (int j = 0; j < currentParent.chromR[i].size() - 1; ++j) {
                routeCost += params.timeCost[currentParent.chromR[i][j]][currentParent.chromR[i][j + 1]];
            }

            // Calcul du bénéfice de la route
            for (int customer : currentParent.chromR[i]) {
                routeBenefit += params.timeCost[0][customer]; // Supposant que l'indice 0 représente le dépôt
            }

            // Calcul du ratio bénéfice/coût
            double ratio = routeBenefit / routeCost;
        ...
        // Vérifier si une route a été sélectionnée
        if (maxRouteIndex != -1) {
            // Transférer la route avec le ratio bénéfice/coût le plus élevé à result.chromR
            result.chromR[resultRouteIndex].clear(); // Nettoyer la route de destination
            for (int customer : currentParent.chromR[maxRouteIndex]) {
                result.chromR[resultRouteIndex].push_back(customer); // Copier les clients de la route sélectionnée
            }
            resultRouteIndex++; // Incrémenter l'index de la route dans result.chromR
        } else {
            break; // Sortir de la boucle si aucune route n'a été trouvée
        }

        // Supprimer tous les clients de result.chromR des deux parents
        for (int i = 0; i < resultRouteIndex; ++i) {
            for (int customer : result.chromR[i]) {
                for (auto &route : modifiedParent1.chromR) {
                    route.erase(std::remove(route.begin(), route.end(), customer), route.end());
                }
                for (auto &route : modifiedParent2.chromR) {
                    route.erase(std::remove(route.begin(), route.end(), customer), route.end());
                }
            }
        }
    ...
    // Vérification: Supprimer les clients dupliqués dans result.chromR
    for (int i = 0; i < resultRouteIndex; ++i) {
        std::sort(result.chromR[i].begin(), result.chromR[i].end());
        result.chromR[i].erase(std::unique(result.chromR[i].begin(), result.chromR[i].end()), result.chromR[i].end());
    }

    // Parcourir result.chromR et comparer avec parent1 pour ajouter les clients manquants
    for (int i = 0; i < params.nbVehicles; ++i) {
        for (int customer : parent1.chromR[i]) {
            bool customerFound = false; // Indicateur pour suivre si le client est trouvé dans result.chromR
            // Parcourir chaque route dans result.chromR
            for (int j = 0; j < resultRouteIndex; ++j) {
                // Vérifier si le client est présent dans la route j de result.chromR
                if (std::find(result.chromR[j].begin(), result.chromR[j].end(), customer) != result.chromR[j].end()) {
                    customerFound = true; // Marquer le client comme trouvé
                    break; // Sortir de la boucle car le client a été trouvé dans result.chromR
                }
            }
            // Si le client n'est pas trouvé dans result.chromR, l'ajouter à la dernière route de result.chromR
            if (!customerFound) {
                result.chromR[resultRouteIndex - 1].push_back(customer);
            }
        }
    }
...
void Genetic::crossoverGrPX2(Individual & result, const Individual & parent1, const Individual & parent2)
{
    ...
    while (true) {
        ...
        const Individual &currentParent = (currentParentIndex == 1) ? modifiedParent1 : modifiedParent2;
/*
        // Affichage des routes du parent avant l'insertion
        std::cout << "Parent " << currentParentIndex << " before insertion:" << std::endl;
        for (int i = 0; i < params.nbVehicles; ++i) {
            std::cout << "Route " << i << ": ";
            for (int customer : currentParent.chromR[i]) {
                std::cout << customer << " ";
...
*/
        // Recherche de la route avec le plus de clients dans le parent courant
        int maxRouteSize = 0;
        int maxRouteIndex = -1;
        for (int i = 0; i < params.nbVehicles; ++i) {
            if (currentParent.chromR[i].size() > maxRouteSize) {
                maxRouteSize = currentParent.chromR[i].size();
                maxRouteIndex = i;
            }
        }

        // Vérifier si aucune route n'a été trouvée (c'est-à-dire toutes les routes sont vides)
        if (maxRouteIndex == -1) {
            break; // Sortie de la boucle si aucune route n'a été trouvée
        }

         // Transférer la partie contenant le plus de clients du parent sélectionné à result.chromR
        result.chromR[resultRouteIndex++] = currentParent.chromR[maxRouteIndex];

        // Supprimer tous les clients de result.chromR des deux parents
        for (int customer : result.chromR[resultRouteIndex - 1]) {
            for (auto &route : modifiedParent1.chromR) {
                route.erase(std::remove(route.begin(), route.end(), customer), route.end());
            }
            for (auto &route : modifiedParent2.chromR) {
        ...
        // Supprimer la route sélectionnée du parent actuel
        if (currentParentIndex == 1) {
            modifiedParent1.chromR[maxRouteIndex] = {};
        } else {
            modifiedParent2.chromR[maxRouteIndex] = {};
        }
/*
        // Affichage des routes du parent après l'insertion
        std::cout << "Parent " << currentParentIndex << " after insertion:" << std::endl;
        for (int i = 0; i < params.nbVehicles; ++i) {
            std::cout << "Route " << i << ": ";
            for (int customer : currentParent.chromR[i]) {
                std::cout << customer << " ";
        ...
        // Affichage du résultat après chaque insertion
        std::cout << "Result after insertion from parent " << currentParentIndex << ":" << std::endl;
        for (int i = 0; i < params.nbVehicles; ++i) {
            std::cout << "Route " << i << ": ";
            for (int customer : result.chromR[i]) {
                std::cout << customer << " ";
